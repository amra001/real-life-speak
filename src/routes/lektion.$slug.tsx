...
export const Route = createFileRoute("/lektion/$slug")({
  head: ({ params }) => ({
    meta: [
      ...
      {
        name: "description",
        content: "Deutsch lernen mit visuellen Alltagsszenen, Übersetzung, Vokabeln, Dialog und Quiz.",
      },
    ],
  }),
  ...
  return (
    <div className="mt-6 rounded-3xl border border-border bg-card p-8 text-center md:p-10">
      ...
      <div className="mt-6 flex flex-wrap justify-center gap-3">
        <Button asChild><Link to="/preise"><Sparkles className="mr-2 h-4 w-4" /> Premium freischalten</Link></Button>
        <Button asChild variant="outline"><Link to="/lektionen">Kostenlose Lektionen</Link></Button>
      </div>
    </div>
  );
}
...
function LessonPage() {
  ...
  if (!data) return (
    <div className="mx-auto max-w-4xl px-4 py-20 text-center">
      ...
      <Button asChild className="mt-6"><Link to="/lektionen">Zur Bibliothek</Link></Button>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      <Link to="/lektionen" className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground">
      ...
      <div className="mt-6 overflow-hidden rounded-3xl border border-border bg-card">
        ...
        {scene && (
          <div className="border-t border-border p-6">
            {!fullAccess && <div className="mb-3 inline-flex rounded-full bg-muted px-3 py-1 text-xs uppercase tracking-widest text-muted-foreground">Kostenlose Vorschau</div>}
            <div className="font-serif text-2xl leading-snug">{scene.german_text}</div>
            {translate(scene.translations) && <div className="mt-2 text-sm text-muted-foreground">{translate(scene.translations)}</div>}
            <div className="mt-5 flex items-center gap-3">
              <Button variant="outline" size="icon" disabled={safeStep === 0} onClick={() => setStep((s) => Math.max(s - 1, 0))} aria-label="Vorherige Szene">
                <ChevronLeft className="h-4 w-4" />
              </Button>
              <Progress value={((safeStep + 1) / Math.max(visibleScenes.length, 1)) * 100} className="flex-1" />
              <span className="text-xs text-muted-foreground">{safeStep + 1}/{visibleScenes.length}</span>
              <Button variant="outline" size="icon" disabled={safeStep >= visibleScenes.length - 1} onClick={() => setStep((s) => s + 1)} aria-label="Nächste Szene">
      ...
      <Tabs defaultValue="vokabeln" className="mt-8">
        ...
        <TabsContent value="dialog" className="mt-5 space-y-3">
          {fullAccess ? <>
            {dialog.map((d) => <div key={d.id} className={d.speaker === "A" ? "max-w-[85%] rounded-2xl rounded-bl-sm border border-border bg-card p-4" : "ml-auto max-w-[85%] rounded-2xl rounded-br-sm bg-muted p-4"}>
              <div className="text-xs uppercase tracking-widest text-muted-foreground">Person {d.speaker}</div>
        ...
        <TabsContent value="quiz" className="mt-5">
          <Quiz questions={visibleQuestions} lessonId={lesson.id} userId={user?.id} locked={!fullAccess && questions.length > visibleQuestions.length} />
        </TabsContent>
      </Tabs>
    </div>
  );
...
function Quiz({ questions, lessonId, userId, locked }: { questions: Question[]; lessonId: string; userId: string | undefined; locked?: boolean }) {
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [texts, setTexts] = useState<Record<string, string>>({});
  const [submitted, setSubmitted] = useState(false);
  const sorted = useMemo(() => questions.map((q) => ({ ...q, quiz_answers: [...q.quiz_answers].sort((a, b) => a.position - b.position) })), [questions]);
  ...
  const isCorrect = (q: (typeof sorted)[number]) => {
    if (isTextKind(q.kind ?? "multiple_choice")) {
      const solution = q.quiz_answers.find((a) => a.is_correct)?.text ?? "";
      return !!solution && (texts[q.id] ?? "").trim().toLowerCase() === solution.trim().toLowerCase();
    }
    return !!q.quiz_answers.find((a) => a.id === answers[q.id])?.is_correct;
  };
  const answeredCount = sorted.filter((q) => isTextKind(q.kind ?? "multiple_choice") ? (texts[q.id] ?? "").trim() : answers[q.id]).length;
  ...
  async function submit() {
    ...
    const { error } = await supabase.from("user_progress").upsert({
      ...
      lesson_id: lessonId,
      completed: true,
      quiz_score: sorted.filter(isCorrect).length,
  ...
  return <div className="space-y-6">
    {sorted.map((q, i) => {
      ...
      return <div key={q.id} className="rounded-2xl border border-border bg-card p-5">
        <div className="flex flex-wrap items-center gap-2 text-xs uppercase tracking-widest text-muted-foreground"><span>Frage {i + 1}</span><Badge variant="outline" className="font-normal normal-case">{KIND_LABEL[kind] ?? "Multiple Choice"}</Badge></div>
        <div className="mt-2 font-medium">{q.prompt}</div>
        {isTextKind(kind) ? <Input className="mt-4" placeholder="Antwort eingeben" value={texts[q.id] ?? ""} disabled={submitted} onChange={(e) => setTexts((p) => ({ ...p, [q.id]: e.target.value }))} /> :
          <div className="mt-4 space-y-2">{q.quiz_answers.map((a) => {
            const selected = answers[q.id] === a.id;
            const state = submitted ? a.is_correct ? "border-success bg-success/10" : selected ? "border-destructive bg-destructive/10" : "border-border" : selected ? "border-foreground" : "border-border";
            return <button key={a.id} disabled={submitted} onClick={() => setAnswers((p) => ({ ...p, [q.id]: a.id }))} className={`block w-full rounded-xl border px-4 py-3 text-left text-sm ${state}`}>{a.text}</button>;
          })}</div>}
        {submitted && <div className="mt-3 space-y-1">
          {isTextKind(kind) && <p className={`text-sm ${correct ? "text-success" : "text-destructive"}`}>{correct ? "Richtig!" : `Richtige Antwort: ${q.quiz_answers.find((a) => a.is_correct)?.text ?? ""}`}</p>}
          {!correct && q.explanation && <p className="text-sm text-muted-foreground">{q.explanation}</p>}
        </div>}
      </div>;
    })}

    {locked && <div className="rounded-2xl border border-dashed border-border p-5 text-sm text-muted-foreground">Das vollständige Quiz ist Teil von Premium. <Link to="/preise" className="font-medium text-foreground hover:underline">Premium freischalten</Link></div>}

    {submitted ? <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-border bg-card p-5">
      <div className="font-serif text-xl">{score} von {sorted.length} richtig</div>
      {!userId && <span className="text-sm text-muted-foreground">Melde dich an, um deinen Fortschritt zu speichern.</span>}
