...
export function StandardLessonPage({ slug }: { slug: string }) {
  const q = useQuery(lessonQuery(slug));
  ...
  if (!data?.lesson) return <div className="mx-auto max-w-5xl px-4 py-20 whitespace-pre-wrap">Lektion nicht gefunden.{`\nDEBUG2 status=${q.status} fetchStatus=${q.fetchStatus} isFetching=${q.isFetching} dataUpdatedAt=${q.dataUpdatedAt} error=${q.error ? String((q.error as any)?.message ?? q.error) : "none"}`}</div>;
