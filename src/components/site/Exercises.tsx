...
export function normalizeQuestion(q: Question, lang: TranslationLang): NormQuestion {
  ...
  const orderItems = items.length >= 3 ? items : answers.length >= 3 ? answers.map((a) => a.text) : [];
...
function isAnswered(q: NormQuestion, s: State): boolean {
  switch (q.kind) {
    ...
    case "gap_select":
      return !!(s.text ?? "").trim();
    case "sentence_order":
      return (s.order ?? []).length === q.items.length;
    case "order":
    case "dialog_order":
      return (s.order ?? []).length === q.items.length;
    case "match":
      ...
      {values.map((v, i) => (
        ...
        >
          ...
          <span className="flex shrink-0 gap-1">
            <Button type="button" variant="outline" size="icon" className="h-7 w-7" disabled={disabled || i === 0} onClick={() => move(i, -1)} aria-label="nach oben">
              <ArrowUp className="h-3.5 w-3.5" />
            </Button>
            <Button type="button" variant="outline" size="icon" className="h-7 w-7" disabled={disabled || i === values.length - 1} onClick={() => move(i, 1)} aria-label="nach unten">
...
function ChipBuilder({ words, value, onChange, disabled }: { words: string[]; value: string[]; onChange: (v: string[]) => void; disabled: boolean }) {
  const pool = useMemo(() => {
    const rest = [...words];
    ...
    <div className="mt-4 space-y-3">
      <div className="min-h-12 rounded-xl border border-dashed border-border p-2">
        <div className="flex flex-wrap gap-2">
          {value.map((w, i) => (
            <button key={`${w}-${i}`} type="button" disabled={disabled} onClick={() => onChange(value.filter((_, j) => j !== i))} className="rounded-full bg-foreground px-3 py-1.5 text-sm text-background">
              {w}
            </button>
          ))}
      ...
      <div className="flex flex-wrap gap-2">
        {pool.map((w, i) => (
          <button key={`${w}-p-${i}`} type="button" disabled={disabled} onClick={() => onChange([...value, w])} className="rounded-full border border-border px-3 py-1.5 text-sm hover:bg-muted">
            {w}
          </button>
        ))}
      </div>
  ...
  return (
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      <div className="space-y-2">
        {pairs.map((p) => (
          <button key={p.left} type="button" disabled={disabled} onClick={() => setActive(active === p.left ? null : p.left)} className={`block w-full rounded-xl border px-3 py-2 text-left text-sm ${active === p.left ? "border-foreground" : "border-border"}`}>
            <span className="font-medium">{p.left}</span>
            {value[p.left] && <span className="mt-0.5 block text-xs text-muted-foreground" dir="auto">→ {value[p.left]}</span>}
          </button>
        ))}
      </div>
      <div className="space-y-2">
        {rights.map((r) => (
          <button
            ...
            onClick={() => {
              if (!active) return;
              onChange({ ...value, [active]: r });
              setActive(null);
            }}
            className={`block w-full rounded-xl border px-3 py-2 text-left text-sm ${used.has(r) ? "border-border bg-muted/60 text-muted-foreground" : "border-border hover:bg-muted"}`}
            dir="auto"
          >
            {r}
          </button>
  ...
  return (
    <div className="space-y-5">
      ...
      {norm.map((q, i) => {
        ...
        return (
          <div key={q.id} className="rounded-2xl border border-border bg-card p-5">
            ...
            {q.kind === "image_choice" && q.imageKey && <img src={lessonImage(q.imageKey)} alt={q.prompt} loading="lazy" className="mt-3 aspect-video w-full rounded-xl object-cover" />}
            ...
            {(q.kind === "single_choice" || q.kind === "true_false" || q.kind === "image_choice") && (
              <div className={q.kind === "true_false" ? "mt-4 flex gap-3" : "mt-4 space-y-2"}>
                {q.answers.map((a) => {
                  ...
                  return (
                    <button key={a.id} type="button" disabled={isChecked} onClick={() => set(q.id, { choice: a.id })} className={`rounded-xl border px-4 py-3 text-left text-sm ${cls} ${q.kind === "true_false" ? "flex-1 text-center font-medium" : "block w-full"}`}>
                      {a.text}
                    </button>
                  );
                })}
            ...
            {q.kind === "multi_choice" && (
              <div className="mt-4 space-y-2">
                {q.answers.map((a) => {
                  ...
                  return (
                    <label key={a.id} className={`flex w-full cursor-pointer items-center gap-3 rounded-xl border px-4 py-3 text-sm ${cls}`}>
                      <Checkbox checked={picked} disabled={isChecked} onCheckedChange={(v) => set(q.id, { multi: v ? [...(s.multi ?? []), a.id] : (s.multi ?? []).filter((x) => x !== a.id) })} />
                      <span>{a.text}</span>
                    </label>
                  );
                })}
            ...
            {q.kind === "gap_select" && (
              <div className="mt-4 flex flex-wrap gap-2">
                {q.options.map((o) => (
                  <button key={o} type="button" disabled={isChecked} onClick={() => set(q.id, { text: o })} className={`rounded-full border px-3 py-1.5 text-sm ${s.text === o ? "border-foreground bg-foreground text-background" : "border-border"}`}>
                    {o}
                  </button>
                ))}
              </div>
            ...
            {q.kind === "sentence_order" && (
              <ChipBuilder words={shuffle(q.items, `${q.id}-pool`)} value={s.order ?? []} disabled={isChecked} onChange={(v) => set(q.id, { order: v })} />
            )}
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              {!isChecked ? (
                <Button type="button" variant="outline" size="sm" disabled={!isAnswered(q, s)} onClick={() => setChecked((p) => ({ ...p, [q.id]: true }))}>Prüfen</Button>
              ) : (
                <>
                  <span className={`inline-flex items-center gap-1 text-sm font-medium ${solved ? "text-success" : "text-destructive"}`}>
                    {solved ? <Check className="h-4 w-4" /> : <X className="h-4 w-4" />}
                    {solved ? "Richtig" : "Noch nicht ganz."}
                  </span>
                  <Button type="button" variant="ghost" size="sm" onClick={() => { setChecked((p) => ({ ...p, [q.id]: false })); set(q.id, initial[q.id] ?? {}); }}>
            ...
            {isChecked && (
              <div className="mt-3 space-y-1 text-sm text-muted-foreground">
                {!solved && (q.kind === "gap" || q.kind === "gap_select") && <p>Richtige Antwort: {q.solutionText}</p>}
                {!solved && (q.kind === "order" || q.kind === "dialog_order" || q.kind === "sentence_order") && <p>Richtige Reihenfolge: {q.items.join(" · ")}</p>}
                {!solved && q.kind === "match" && <p>{q.pairs.map((p) => `${p.left} ↔ ${p.right}`).join(" · ")}</p>}
      ...
      <div className="flex flex-wrap items-center gap-4 rounded-2xl border border-border bg-card p-5">
        ...
        <Button className="ml-auto" disabled={!allChecked || done} onClick={() => { setDone(true); onFinish?.(score, norm.length); }}>
          {done ? "Gespeichert" : "Ergebnis speichern"}
        </Button>
        <Button variant="outline" onClick={() => { setState(initial); setChecked({}); setDone(false); }}>
